    ...
    methods: {
        addPost() {
            axios.get(`${this.baseUrl}/addPost`, {
                params: { 'subject': this.subject, 'entry': this.entry, 'mood': this.selEmotion }
            })
                .then(response => {
...
<template>
    <div class="table m-2">
        ...
        <select v-model="selEmotion">
            <option :value="emotion" v-for="emotion of emotions">{{ emotion }}</option>
        </select>
